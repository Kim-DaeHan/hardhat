...
contract NftMint {
    ...
    function mint(
        ...
        ownerOf[tokenId] = mintContract;
        balanceOf[mintContract]++;
        tokenData[tokenId] = TokenData(name, symbol, 18, description);

        approve(transferContract, mintContract, tokenId);
        transfer(transferContract, mintContract, tokenId);

        emit Minted(transferContract, tokenId);
        return true;
    }
    ...
    function transfer(
        ...
        require(
            ownerAddress == _owner || allowance[ownerAddress][to] >= tokenId,
            "Not authorized"
        );
